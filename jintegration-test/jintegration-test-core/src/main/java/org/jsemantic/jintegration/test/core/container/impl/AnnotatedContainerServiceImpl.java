...
import org.jsemantic.jicontenedor.core.contenedor.Container;
import org.jsemantic.jicontenedor.core.contenedor.configuration.ContainerFactory;
...
import org.jsemantic.jiservice.core.component.exception.ComponentException;
import org.jsemantic.jiservice.core.exception.CycleException;
import org.jsemantic.jiservice.core.service.Service;
import org.jsemantic.jiservice.core.service.exception.ServiceException;
import org.jsemantic.jiservice.core.service.skeletal.AbstractService;
...
public class AnnotatedContainerServiceImpl extends AbstractService implements
	...
	protected void postConstruct() throws CycleException {
		contenedor = ContainerFactory.getInstance(CONFIGURATION_FILE);
		contenedor.start();
	...
	private void processAnnotatedServices(Class<?> testClass) {
		...
		if (testClass
			...
			JettyService httpService = (JettyService) contenedor
					.getService(JettyService.HTTP_SERVICE_ID);
			if (httpService != null) {
				...
				boolean processed = processor.processAnnotation(httpService,
						testClass);
				if (processed) {
					services.put(JettyService.HTTP_SERVICE_ID, httpService);
				}
			}
		}
